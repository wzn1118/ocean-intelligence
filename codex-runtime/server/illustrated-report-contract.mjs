...
export function illustratedReportInstructions(contract) {
  ...
  return [
    ...
    'For interactive HTML, keep one unambiguous application/json point model. Use canonical observation_id, temperature, unit, time, longitude, latitude and qc fields on every record; preserve original source fields and all other quantities/QC alongside these aliases without changing values or units. MATLAB table names such as ObservationID, Time and TemperatureUnit are not the HTML wire format. Each rendered point needs matching data-observation-id and a zero-based data-point-index into that exact JSON array; keep one-based source rows in separate source_row/source_file_row fields and retain an explicit permutation after sorting or filtering.',
    'Bind actual point elements to pointerenter and focus handlers, with visible :hover and :focus-visible states, keyboard focusability and a real tooltip/detail view. Each rendered point must retain data-temperature, data-unit, data-time, data-longitude, data-latitude and data-qc for the same record. Verify real mouse, keyboard and touch behavior across desktop/mobile, including dense overlapping points, tooltip clipping and focus/pointer conflicts. Passing the static point inspector proves neither that a handler displays correct values nor that a browser or MATLAB interaction ran.',
    'Use present, absent, unknown or not-evaluated for data-anomaly-status; do not substitute not_assessed. Keep unavailable runtime, export and visual fields pending or unverified. A successful DOM or browser check must never be used to invent MATLAB release, execution, artifact-validation or visual-inspection approval.',

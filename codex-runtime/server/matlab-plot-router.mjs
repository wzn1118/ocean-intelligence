...
- 绘图前的 figure/layout 仍按像素/DPI 设置最终 inches，不把屏幕像素作为输出尺寸。runtime.export_size_units 按实际路径记录：原生 PNG、print PNG、PDF 及请求的 SVG 均为 inches。不采用会缩小字体并增加刻度的 pixels/off 路径；inches/off 的有限探针不能替代全量验收。不做导出后 resize，不通过重采样、裁切或填边掩盖尺寸错误；本次 PNG inches/off 策略尚待跨版本全量 CI 验证，不得声称尺寸偏差已经修复或视觉满分。

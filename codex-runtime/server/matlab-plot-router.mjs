...
- 本生成器固定调用 oi_export_figure 严格尺寸审计资产，并向任务预检声明 auditedFigureManifest：R2019b-R2024b 的 PNG/PDF/SVG 均使用明确声明的 print 回退（SVG 为 print -dsvg），因为旧版原生 tight 裁切不能保证指定像素和页尺寸；R2025a 起使用 exportgraphics，PNG 使用 Units="inches"、Width=widthPixels/dpi、Height=heightPixels/dpi、Resolution=dpi 和 PreserveAspectRatio="off"；PDF/SVG 使用相同物理尺寸的 Units="inches"、Width=widthPixels/dpi、Height=heightPixels/dpi 和 PreserveAspectRatio="on"；两类均保留 Padding="figure"。R2020a-R2024b 仍有 exportgraphics，缺少的是严格尺寸参数；apiPlan.export 仅说明通用 API 可用性，实际目标策略见 apiPlan.exportFormats 与 headless.exportApis，不改写其他一般路由的通用能力。
- 绘图前的 figure/layout 仍按像素/DPI 设置最终 inches，不把屏幕像素作为输出尺寸。runtime.export_size_units 按实际路径记录：原生 PNG、print PNG、PDF 及请求的 SVG 均为 inches。不采用会缩小字体并增加刻度的 pixels/off 路径。第12-16轮已有 R2021a/R2024b/R2026a 三版原生尺寸证据（含 R2026a PNG inches/off），3 组 circle 局部像素宽高差不超过 2 个边缘像素。已有尺寸和局部证据不代表本次任务实跑或整体 CI 通过；全图视觉尚未通过，每次新图仍须逐格式验证尺寸/DPI、字体、刻度和裁切。不做导出后 resize，不通过重采样、裁切或填边掩盖尺寸错误。

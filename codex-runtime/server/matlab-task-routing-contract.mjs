...
export function selectMatlabAuditedExportStrategy(targetRelease, format) {
  ...
  if (exactSizing) {
    ...
    const geometryArgs = "'Units','inches','Width',widthPixels/dpi,'Height',heightPixels/dpi";
    const preserveAspectRatio = plan.format === 'png' ? 'off' : 'on';
    return { ...plan, ...metadata,
      syntax: `exportgraphics(figureHandle,file,${geometryArgs},'Padding','figure','PreserveAspectRatio','${preserveAspectRatio}','BackgroundColor','white'${formatArgs})` };
  }
...
- requestedCapabilities 显式包含 auditedFigureManifest 时，选择 oi_export_figure 的严格尺寸审计路径：必须保留 manifest 与 PNG/PDF，可选 SVG；R2019b-R2024b 声明 print，R2025a 起声明 exportgraphics：PNG 使用 Units inches、Width=widthPixels/dpi、Height=heightPixels/dpi、Resolution=dpi 和 PreserveAspectRatio off；PDF/SVG 使用相同物理 Width/Height 的 Units inches 和 PreserveAspectRatio on，均保留 Padding figure。逐格式 export_size_units 记录实际请求的 inches，目标策略不等于已经实跑通过；不采用会缩小字体并增加刻度的 pixels/off 路径。第12-16轮已有 R2021a/R2024b/R2026a 三版原生尺寸证据（含 R2026a PNG inches/off），3 组 circle 局部像素宽高差不超过 2 个边缘像素。已有尺寸和局部证据不代表本次任务实跑或整体 CI 通过；全图视觉尚未通过，每次新图仍须逐格式验证尺寸/DPI、字体、刻度和裁切。旧版 exportgraphics 仍可能存在，只是缺少严格尺寸参数。仅要求 manifest 或出版物理尺寸不改变一般路由的通用 API 策略。

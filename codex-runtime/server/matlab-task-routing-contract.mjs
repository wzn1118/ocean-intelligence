...
- requestedCapabilities 显式包含 auditedFigureManifest 时，选择 oi_export_figure 的严格尺寸审计路径：必须保留 manifest 与 PNG/PDF，可选 SVG；R2019b-R2024b 声明 print，R2025a 起声明 exportgraphics：PNG 使用 Units inches、Width=widthPixels/dpi、Height=heightPixels/dpi、Resolution=dpi 和 PreserveAspectRatio off；PDF/SVG 使用相同物理 Width/Height 的 Units inches 和 PreserveAspectRatio on，均保留 Padding figure。逐格式 export_size_units 记录实际请求的 inches，目标策略不等于已经实跑通过；新 PNG inches/off 策略尚待跨版本全量 CI 验证，不得声称已修复或视觉满分，不采用会缩小字体并增加刻度的 pixels/off 路径。旧版 exportgraphics 仍可能存在，只是缺少严格尺寸参数。仅要求 manifest 或出版物理尺寸不改变一般路由的通用 API 策略。

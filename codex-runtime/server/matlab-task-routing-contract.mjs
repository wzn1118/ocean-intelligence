...
export function selectMatlabAuditedExportStrategy(targetRelease, format) {
  ...
  if (exactSizing) {
    ...
    const geometryArgs = plan.format === 'png'
      ? "'Units','pixels','Width',widthPixels,'Height',heightPixels"
      : "'Units','inches','Width',widthPixels/dpi,'Height',heightPixels/dpi";
    return { ...plan, ...metadata,
      syntax: `exportgraphics(figureHandle,file,${geometryArgs},'Padding','figure','PreserveAspectRatio','on','BackgroundColor','white'${formatArgs})` };
  }
...
- requestedCapabilities 显式包含 auditedFigureManifest 时，选择 oi_export_figure 的严格尺寸审计路径：必须保留 manifest 与 PNG/PDF，可选 SVG；R2019b-R2024b 声明 print，R2025a 起声明 exportgraphics：PNG 使用 Units pixels、整数 Width/Height 和 Resolution；PDF/SVG 使用 Units inches 和物理 Width/Height，均保留 Padding figure、PreserveAspectRatio on。逐格式 export_size_units 记录实际请求，目标策略不等于已经实跑通过。旧版 exportgraphics 仍可能存在，只是缺少严格尺寸参数。仅要求 manifest 或出版物理尺寸不改变一般路由的通用 API 策略。

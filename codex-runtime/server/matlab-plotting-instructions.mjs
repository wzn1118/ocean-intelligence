...
本段仅适用于 MATLAB 仓库审计导出，与前述 MATLAB helper、字体和布局建议共同生效。通用 release 能力矩阵说明 API 是否可用，不代表本仓库固定尺寸与 manifest 验证已通过。以下区分已有产物证据与待验代码候选，不代表本次会话已执行、已部署或既有会话热更新。
...
- runtime.export_size_units 按实际路径记录：原生 PNG、print PNG、PDF 及请求的 SVG 均为 inches。inches/off 保留物理字体且所测 3/3 尺寸准确的初始探针，已有包含 R2026a PNG inches/off 的三版全量原生尺寸回归补充；第15/16轮也已有 PNG/SVG 尺寸实跑证据，不再仅是待首次全量验证的策略。三幅 unit-circle 的局部像素宽高差不超过 2 个边缘像素，不是全图视觉保证；pixels/off 实图字体缩小、刻度变多，不采用该路径。不做导出后 resize，不通过重采样、裁切或填边掩盖尺寸错误；必须重新检查真实 PNG 像素/DPI、字体、刻度、裁切、PDF 页尺寸及 SVG 几何，未验证项保持 unverified。
- oi_annotate_svg 只对受支持的 SVG 子集做受限嵌套 viewport 规范化，保留内部原生 viewBox 和绘图坐标；这是原生导出后的 XML 后处理，不是未经处理的纯原生 SVG。未知或不支持的 SVG 必须拒绝，不得泛化覆盖或降低尺寸门禁。第15轮命名空间明确的 MATLAB Java DOM 检查在 R2021a/R2024b/R2026a 通过（10 个正例、34 个拒绝例），主阶段 60/60 通过但后处理和整体 CI 仍失败。两套 SVG 布局引擎的历史副本对照零像素差、CairoSVG 对四件实际输出的对照一致，但共用 Cairo 后端，不是独立浏览器、字体或全图视觉验收。
- 历史第16轮 33995525791 三版各 19/20 主阶段通过，family-b-runtime 因 Legend.Title 不支持 FontUnits 失败，整体 CI 仍失败。三版 artifact_validation.verified 修复已有证据，runtime contract 仅剩 visual_inspection.required 失败；R2026a 的十二产物外检通过也不等于视觉或整体 CI 通过，旧版外部 PDF 字体失败仍须保留。
- 最近已完成的第20轮 34000171748（远端 31e74db52922031dfe1f15b7f385c38e620a9d7f）：R2021a 为 19/20，R2024b/R2026a 各 20/20，合计 59/60，evaluator-runtime 三版 passed；各自 evaluator-result.json 原始评分为 90、状态仍为 runtime_pending，整体 CI 仍失败，视觉未通过。唯一主阶段失败是 R2021a comparison-statistics-layout 的 test_comparison_statistics_layout:NativeSubtitle：创建时 Subtitle 为 10 points，扰动 axes 至 12 后，restyle 将 axes 恢复为 10，但 Subtitle 仍为 12。第21轮 oi_apply_axes 候选显式将 Subtitle 设为 theme.FontSize，仍待新 licensed CI，原失败字号断言不放宽。
- 旧的两个 simple canvas 诊断（0pt/3pt inset）已在 R2021a/R2024b 的 primary 和 DISPLAY 中取得 geometry_before_pdf、geometry_after_pdf、geometry_after_png 的 captured 快照；采集成功不等于几何不变、恢复成功或视觉通过。新的四 fixture test_native_pdf_fixture_canvas 诊断尚未 MATLAB 执行；它和附加 simple canvas 均不计入原三 candidate 的 native PDF probe stage、评分或正式报告产物。
...
- scientific_data_contract.plot_data_evidence 的旧 v2 归档覆盖为 3/4 图：温度场、盐度剖面，以及第13轮三版通过的 paired-interactive v2 完整值/QC/不确定度/errorbar 数组；旧包的比较散点仍为 not_verified。第18/19/20轮三版 report-evidence.json 的 runtime_evidence.figures 四图的 plot_data_evidence.status 均为 runtime_declaration_verified，最新原生证据覆盖为 4/4；不得据此升级旧包。报告必须逐项核对完整数组、顺序、缺失掩码、单位、策略、release 和输入哈希。仅输入字节绑定且声明匹配时标记 runtime_declaration_verified；缺证据仍为 not_verified，不向其他图外推。metadata 不是误差带，保留 suspect 不是 QC 筛选，声明核对不是独立重执行或视觉验证，也不是桌面交互验证。
- paired-observation-model v3（schema_version=3）在第18轮首次完成三版同图原生数据绑定：run_matlab_gate 已接入观测侧不确定度与 RecordMetadata，读取原生 Scatter 坐标、水平 Line 端点、归属和 UserData 身份。报告/evaluator 消费者核对完整 12 条合成记录、11 对散点、未绘值、QC/不确定度掩码、统计、release 和输入哈希；模型 QC/不确定度明确为 not_provided。消费者 mutation tests 与原生 reader 套件分别计证；仅凭 4/4 声明绑定不能证明原生篡改全过、真实海域报告或全图视觉通过。无 v3 的旧包保持兼容且比较图未验证，畸形或不匹配声明必须失败，不得绕过校验。
- 第20轮 test_comparison_native_evidence 在 R2021a/R2024b/R2026a 每版完成 4 个正例及 36/36 个 reader 负例；实际 native-reader-test-results.json 与日志终标记 COMPARISON_NATIVE_READER_TEST_NEGATIVES=36、COMPARISON_NATIVE_READER_TEST=passed_synthetic_native_mutations_only 一致。scatter-nan-size 已到达 reader 并抛出 run_matlab_gate:ComparisonProofHandles，不能把 setter 失败当 reader 拒绝。最终恢复/哈希断言已到达，original_artifacts_unchanged=true 与六件导出和输入快照哈希一致；仍为 visual_verified=false、desktop_interaction_verified=false。这仅证明该合成原生套件，不升级第19轮仅 5/36 的旧证据，不代表整体 CI 或视觉通过。
- 报告通过显式 --rendered-audit 文件接收外部检查声明；核对审计文件 bytes/SHA-256、manifest/产物绑定、检查条件与状态一致性，不自动发现文件。shell 工作流先执行 rendered-artifact 检查，再把该文件显式传给报告。不传选项保持 not_verified；显式指定的文件缺失、畸形或不一致必须拒绝。报告不独立重跑或认证检查工具，这些声明不是可信视觉审计；旧版 pdf_font_embedding=failed（含未嵌入 Courier）必须显示，不能被未验证的文本或视觉项掩盖。报告构建成功、文本可提取都不等于字形可读或视觉通过。
- 第20轮 R2026a 外部产物检查 12/12 通过，R2021a/R2024b 各有 4 件 PDF 的 pdf_font_embedding 失败；报告保留这些失败，外检通过不等于整体 CI 通过或可信视觉审查。独立 DISPLAY 诊断中，R2024b 仍以 oi_annotate_svg:UnsupportedNormalization 拒绝白名单外的 SVG font 元素，不能把该诊断与 evaluator 产物混为一谈。
- R20 源码候选采用 parseOceanEvidenceTime 共享严格 UTC coverage 端点解析，主报告 data-uncertainty-status/data-uncertainty-method 必须与 manifest 科学上下文精确一致；非空 data-uncertainty 自然说明仍需人审，不能用子串命中认证语义。源码合同不代表真实海区报告已验证，也不扩展为每条原始 point 时间认证。

...
- runtime.export_size_units 按实际路径记录：原生 PNG、print PNG、PDF 及请求的 SVG 均为 inches。第11轮有限探针中 inches/off 保留物理字体且所测 3/3 尺寸准确；pixels/off 实图字体缩小、刻度变多，不采用该路径。本次 PNG inches/off 策略尚待跨版本全量 CI 验证，不得声称尺寸偏差已经修复或视觉满分。不做导出后 resize，不通过重采样、裁切或填边掩盖尺寸错误；必须重新检查真实 PNG 像素/DPI、字体、刻度、裁切、PDF 页尺寸及 SVG 几何，未验证项保持 unverified。

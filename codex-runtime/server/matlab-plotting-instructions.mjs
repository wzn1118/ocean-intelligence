...
- 最近第17轮 33996694221 三版各 18/20，合计 54/60；family-b-runtime 与 evaluator-runtime 因观测侧水平 Line 缺少明确语义标记，被 oi_export_figure:ColorAccessibility 拒绝。未生成完整 evaluator 运行记录或新海区报告，原始评分均为 0，不能拼接历史产物或沿用上一轮 90 分。当前 helper 仅在其实际创建的不确定度线写入已有 OI_ColorAccessibilityRole="uncertainty"，不改变数据、掩码或审计算法；该候选及新负例仍待下一轮实跑，不能用角色标记豁免任意数据线。
...
- oi_plot_comparison 的可选 RecordMetadata 仅适用于 numeric row-aligned 输入，不适用于 table/timetable 配对。它必须是 scalar struct，且只能包含 ID、Time、Depth、DepthUnit、DepthDirection：ID 是每输入行一个、唯一非空的 string 向量；Time 是逐行对齐、非 NaT 的 UTC datetime 向量；Depth 是逐行对齐、实数有限非负的 numeric 向量，DepthUnit="m"、DepthDirection="positive_down"。显式 SampleLabels 必须与全部 ID 一致，不能同时使用 SampleLabelVariable。result.RecordData 保留全部原始行、观测/模型值、时间、深度和身份，result.QC 保留实际提供的 flags 或 not_provided；原生 Scatter/水平 Line 的 UserData 绑定选中 RecordID、SourceRow 和 SourceRowOrigin="call_entry_order"。省略 RecordMetadata 保持原 numeric/tabular 调用兼容，不造记录身份，也不生成 RecordData。
...
- scientific_data_contract.plot_data_evidence 的最近已确认原生证据覆盖为 3/4 图：温度场、盐度剖面，以及第13轮三版通过的 paired-interactive v2 完整值/QC/不确定度/errorbar 数组；旧包的比较散点仍为 not_verified。报告必须逐项核对完整数组、顺序、缺失掩码、单位、策略、release 和输入哈希。仅输入字节绑定且声明匹配时标记 runtime_declaration_verified；缺证据仍为 not_verified，不向其他图外推。metadata 不是误差带，保留 suspect 不是 QC 筛选，声明核对不是独立重执行或视觉验证，也不是桌面交互验证。
- paired-observation-model v3 是当前代码候选：run_matlab_gate 已接入观测侧不确定度与 RecordMetadata，读取原生 Scatter 坐标、水平 Line 端点、归属和 UserData 身份。报告/evaluator 消费者及 mutation tests 已实现对完整 12 条合成记录、11 对散点、未绘值、QC/不确定度掩码、统计、release 和输入哈希的核对；模型 QC/不确定度明确为 not_provided。第17轮 33996694221 在导出阶段失败，尚未执行 v3 读取与原生篡改测试；角色修复候选仍待下一轮 licensed CI，不得把本地测试通过写成 4/4 已实跑。无 v3 的旧包保持兼容且比较图未验证，畸形或不匹配声明必须失败，不得绕过校验。

...
- 最近已完成的第18轮 33997547843：R2021a/R2024b/R2026a 各 19/20，合计 57/60，evaluator-runtime 三版 passed；各自 evaluator-result.json 原始评分为 90、状态仍为 runtime_pending，不是整体 CI 或视觉通过。family-b-runtime 止于 test_comparison_record_metadata:MissingRejection（SampleLabels）：测试误将与 ID 一致的 cellstr 标签列为负例。第19轮仅修正标签测试，仍待新 licensed CI；后续 test_comparison_native_evidence 原生篡改测试尚未进入，不能宣称全过。
...
- 第18轮 R2026a 外部产物检查 12/12 通过，R2021a/R2024b 各有 4 件 PDF 的 pdf_font_embedding 失败；报告保留这些失败。独立 DISPLAY 诊断中，R2024b 仍以 oi_annotate_svg:UnsupportedNormalization 拒绝白名单外的 SVG font 元素，不能把该诊断与 evaluator 产物混为一谈。

...
- 最近已完成的第19轮 33999054663（远端 85ab9d20）：R2021a/R2024b/R2026a 各 19/20，合计 57/60，evaluator-runtime 三版 passed；各自 evaluator-result.json 原始评分为 90、状态仍为 runtime_pending，不是整体 CI 或视觉通过。SampleLabels/cellstr metadata 测试已通过；首次 test_comparison_native_evidence 对抗套件每版仅完成 4 个正例及 5/36 个 reader 负例。随后 Scatter.SizeData=0 在 setter 处抛出 MATLAB:hg:shaped_arrays:PositiveOrNanVectorDataPredicate，reader 尚未被调用，不能算作 reader 拒绝；剩余用例、最终恢复/原件哈希断言和套件成功标记未到达。R20 替换为 NaN 的候选仍待 native 实跑，不得宣称完整套件通过。
...
- scientific_data_contract.plot_data_evidence 的旧 v2 归档覆盖为 3/4 图：温度场、盐度剖面，以及第13轮三版通过的 paired-interactive v2 完整值/QC/不确定度/errorbar 数组；旧包的比较散点仍为 not_verified。第18/19轮三版 report-evidence.json 的 runtime_evidence.figures 四图的 plot_data_evidence.status 均为 runtime_declaration_verified，最新原生证据覆盖为 4/4；不得据此升级旧包。报告必须逐项核对完整数组、顺序、缺失掩码、单位、策略、release 和输入哈希。仅输入字节绑定且声明匹配时标记 runtime_declaration_verified；缺证据仍为 not_verified，不向其他图外推。metadata 不是误差带，保留 suspect 不是 QC 筛选，声明核对不是独立重执行或视觉验证，也不是桌面交互验证。
- paired-observation-model v3（schema_version=3）在第18轮首次完成三版同图原生数据绑定：run_matlab_gate 已接入观测侧不确定度与 RecordMetadata，读取原生 Scatter 坐标、水平 Line 端点、归属和 UserData 身份。报告/evaluator 消费者核对完整 12 条合成记录、11 对散点、未绘值、QC/不确定度掩码、统计、release 和输入哈希；模型 QC/不确定度明确为 not_provided。消费者 mutation tests 与第19轮仅部分执行的原生对抗套件必须区分；4/4 声明绑定不能冒充原生篡改全过、真实海域报告或全图视觉通过。无 v3 的旧包保持兼容且比较图未验证，畸形或不匹配声明必须失败，不得绕过校验。
...
- 第19轮 R2026a 外部产物检查 12/12 通过，R2021a/R2024b 各有 4 件 PDF 的 pdf_font_embedding 失败；报告保留这些失败，外检通过不等于整体 CI 通过或可信视觉审查。独立 DISPLAY 诊断中，R2024b 仍以 oi_annotate_svg:UnsupportedNormalization 拒绝白名单外的 SVG font 元素，不能把该诊断与 evaluator 产物混为一谈。

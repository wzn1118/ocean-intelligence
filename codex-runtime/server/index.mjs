...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (request.method === 'POST' && turnMatch) {
      ...
      const reportMatlabDirectory = report ? path.join(tenant.generatedRoot, `${report.id}-matlab`) : null;
      const turnText = report ? [
        ...
        `Write new MATLAB function source files directly inside ${reportMatlabDirectory}, for example plot_report.m with primary function plot_report. Each primary function must have a legal MATLAB name matching its filename basename; do not put the hyphenated report id into a function name. This source directory is fixed by the server-bound report id, not a caller-provided path.`,
        'The MATLAB source directory accepts only direct regular .m files: non-keyword ASCII basenames of 1-63 characters, starting with a letter and containing only letters, digits or underscores before the lowercase .m extension. Do not place subdirectories, symlinks, hardlinks, data, README or export files there. A filename passing discovery is not proof of valid MATLAB syntax or execution; verify which resolves the function to this report before running it, without adding other reports to the MATLAB path.',
        `Legacy .m scripts directly under ${tenant.generatedRoot} whose filenames begin ${report.id}- remain compatible evidence; this does not require new functions to use that prefix or certify legacy files as callable same-name functions. Keep manifest source references relative to the generated root, including ${report.id}-matlab/plot_report.m for the new layout. Do not borrow another report's scripts or manifest.`,
        ...
        'A bounded real-source replay is not a complete ocean-region report. Reconcile platform, profile and layer/sample counts against this run\'s actual inputs; distinguish discontinuous sampling from continuous coverage and never count layers as independent platforms. Preserve input bytes/hash, source order, UTC/position, each variable\'s original mode and units, and all original QC fields; a declared joint acceptance mask does not replace them. Pressure is not depth. Do not invent uncertainty, density, named-region or nine-zone coverage, independent in-situ validation, request windows or upstream acquisition receipts. Unknown metadata remains unknown; an update timestamp is not a fetch receipt.',
        'Only when generating a pressure-colored T-S plot, use oi_plot_ts_diagram with explicit ColorLabel="Pressure", ColorUnit equal to the source pressure unit, and ColorLimits; unknown SalinityType/TemperatureType remain "unspecified" with source definitions. It has no RawRecords, RecordMetadata or comparison-v3 identity interface. The caller must preserve full records and explicitly map any profile-local source rows to concatenated call rows, bind identities in caller-managed UserData on each actual Scatter selected by CompleteMask, and read back XData/YData/CData, ownership, visibility, order and identities before/after export against the source. SampleLabels or copied input arrays do not prove native identity; a new T-S plot must not claim comparison v3 coverage.',
        'Without actual MATLAB exports or required report evidence, retain complete=false and do not fabricate PNG/PDF, native proof, visual approval or a passing CI result. Keep local projection, each input/release execution, synthetic benchmark scores and full-report/visual acceptance separate; a limited dataset or one successful figure does not satisfy the full report gates.',
      ].join('\n') : text;
      const params = {
        threadId,

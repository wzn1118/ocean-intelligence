...
for seriesIndex = 1:numel(valueVariables)
    ...
    lineHandles(seriesIndex) = plot(axesHandle,plotTimes,plotValues, ...
        ...
        "LineStyle",theme.LineStyles(seriesIndex),"LineWidth",1.3, ...
    ...
    if isprop(lineHandles(seriesIndex),"DataTipTemplate")
        if verLessThan("matlab","9.11")
            lineHandles(seriesIndex).DataTipTemplate.DataTipRows = [ ...
                dataTipTextRow(char(timeLabel + " (" + displayTimeZone + ")"),"XData"); ...
                dataTipTextRow(char(valueLabels(seriesIndex) + " (" + valueUnits(seriesIndex) + ")"),"YData")];
        else
...
uncertaintyType = replace(uncertaintyType,"_","-");
allowedTypes = ["standard-uncertainty" "standard-deviation" "standard-error" "confidence-interval" ...

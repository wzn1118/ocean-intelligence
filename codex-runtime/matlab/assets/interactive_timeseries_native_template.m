...
uncertainty_type = strtrim(options.UncertaintyType);
...
if has_uncertainty || has_lower_bound
  ...
  allowed_uncertainty_types = ["standard-deviation" "standard-error" ...
...
configure_series(axes_handles(1), line_handles(1), point_metadata, value, ...
  value_label, value_unit, theme, font_name);
if has_secondary
...
function configure_series(axes_handle, line_handle, metadata, plotted_values, quantity_label, quantity_unit, theme, font_name)
grid(axes_handle, 'on');
box(axes_handle, 'on');
...
text_rows = {sprintf('Observation ID: %s', char(observation_id)), ...
  sprintf('Source row: %d', metadata.SourceRow(data_index)), ...

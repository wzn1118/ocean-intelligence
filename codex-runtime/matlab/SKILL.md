...
Round-16 run 33995525791 completed 19/20 primary stages on each of R2021a/R2024b/R2026a; `family-b-runtime` failed because `Legend.Title` is `matlab.graphics.illustration.legend.Text`, which does not support `FontUnits`. The current source removes that setter and uses its points-based `FontSize`; successful licensed validation of this correction is still pending. All three releases reflect the `artifact_validation.verified` fix: their runtime-contract violations now contain only `visual_inspection.required`. That does not resolve the separate external PDF failures, including unembedded Courier on the older releases. R2026a passed all twelve external evaluator artifact checks, not the visual gate or overall CI.
...
Round 13 passed the `paired-interactive` version-2 native evidence checks on R2021a/R2024b/R2026a, including complete values, QC, uncertainty, and errorbar arrays. The latest licensed report native-proof coverage remains 3/4 figures; the comparison scatter in those existing packages remains `not_verified`. This verifies input-bound runtime declarations, not desktop interaction, full-figure appearance, or live service updates.

The round-17 candidate now wires `paired-observation-model` into `run_matlab_gate.m` with observation-only uncertainty and `RecordMetadata`, producing v3 evidence by reading native Scatter coordinates, horizontal Line endpoints, ownership and `UserData` identities. The report/evaluator consumers and mutation tests check all 12 synthetic fixture records, all 11 scatter pairs, unplotted values, QC and uncertainty masks, statistics, release and input hashes. Model QC and uncertainty remain explicitly `not_provided`. This producer/consumer path has not yet completed its first licensed CI run: synthetic passing tests do not establish executed 4/4 coverage. Existing packages without v3 remain compatible and unverified for comparison; malformed or mismatched declarations must fail, not bypass validation. The fixtures remain synthetic, not observed ocean evidence.

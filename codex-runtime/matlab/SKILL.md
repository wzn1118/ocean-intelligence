...
Current evidence: round-21 run 34001173593 (remote commit `7d51e714`) completed with overall `completed/failure`. R2021a/R2024b/R2026a each passed 20/20 primary stages, 60/60 in total; `evaluator-runtime` passed on all three. Each original evaluator result still has score 90 and status `runtime_pending`; visual inspection remains unverified. `oi_apply_axes` explicitly applies `theme.FontSize` to Subtitle, and the `comparison-statistics-layout` subtitle perturbation/restyle assertion now passed natively, without relaxing its font-size check. The older failure remains historical evidence, not a current primary-stage failure. Read `evals/README.md` for the run history and diagnostic limits rather than injecting that history into every plotting task.

The four-fixture `test_native_pdf_fixture_canvas` diagnostic ran in both primary and DISPLAY contexts: R2021a's four cases failed with `build_native_pdf_fixture_case:JVMRequired`; R2024b's four cases failed with `test_native_pdf_fixture_canvas:RootObjects`, involving the empty `scribeOverlay` `AnnotationPane`. Some R2024b geometry records also contain `MATLAB:class:GetProhibited`, so they are not complete measurements. Both releases retain `canvas_pdf.api_invoked=false`; R2026a is `not_applicable`, not passed. The round-22 candidate removes the JVM dependency from snapshot SHA and excludes only strictly empty AnnotationPane objects, rejecting nonempty ones; it has not run natively and does not establish a geometry or canvas-PDF fix. Older simple-canvas (0pt/3pt) `geometry_before_pdf`/`geometry_after_pdf`/`geometry_after_png` snapshots were `captured`, not proof of unchanged geometry or restoration. Neither diagnostic counts toward the original three-candidate native PDF probe stage, score, or promoted report artifacts.

...
Round-17 run 33996694221 completed with overall CI failure: 18/20 primary stages on each of R2021a/R2024b/R2026a, 54/60 in total. Both `family-b-runtime` and `evaluator-runtime` failed with `oi_export_figure:ColorAccessibility` on all three releases: the new observation-only uncertainty segments had `HandleVisibility="off"` but no explicit audit role. Each original evaluator score is 0 because `evaluator-runtime/matlab-runtime.json` was not produced; there is no new comparison v3 evidence or complete report. Do not carry forward earlier 90-point scores or 3/4 native-proof coverage as this run's result. All three releases got past the old `Legend.Title.FontUnits` error and passed the four legend-title text-bounds cases per release. These are scoped runtime results, not visual approval. Historical PDF failures, including unembedded Courier on older releases, remain unresolved.
...
For comparison data with uncertainty only on observations, `oi_plot_comparison` accepts explicit `UncertaintySides="observation"`, `ObservationUncertainty` or its variable selector, and `UncertaintyType="standard-uncertainty"` with the actual quantity unit. Omit model uncertainty inputs entirely; never substitute zeros or copy observation uncertainty. Missing observation uncertainty preserves finite QC-accepted scatter pairs and their statistics, but produces no horizontal interval. The helper returns the aligned values, provided/not-provided sides and actual `GraphicsMask` in `result.Uncertainty`; its native legend title identifies the horizontal intervals and absent model uncertainty. Default `UncertaintySides="both"` keeps the existing two-sided contract. The round-18 candidate sets existing audit appdata `OI_ColorAccessibilityRole="uncertainty"` on the actual helper-created uncertainty Lines, without changing the audit algorithm, data, dimension gates or visual gates. This role annotation still awaits round-18 licensed CI; `HandleVisibility="off"` alone must never exempt finite data from auditing.

Round 17 adds optional strict `RecordMetadata` only for numeric row-aligned comparison inputs, not table/timetable pairing. Its scalar struct must contain exactly `ID`, `Time`, `Depth`, `DepthUnit`, and `DepthDirection`: one unique nonblank string ID, non-NaT UTC datetime, and finite nonnegative depth per input row, with `m` and `positive_down`. Explicit `SampleLabels` must match the IDs; `SampleLabelVariable` is not accepted with this metadata. `result.RecordData` retains every original row, value and identity, `result.QC` retains supplied flags or `not_provided`, and native Scatter/horizontal Line `UserData` carries selected record IDs and call-entry source rows. Omitting `RecordMetadata` preserves existing numeric and tabular calling contracts without inventing record identity or emitting `RecordData`. The first licensed attempt reached the comparison export but failed its color-accessibility gate on all three releases; successful end-to-end evidence is still absent. Do not fabricate model QC or model uncertainty when only observations supply them.
...
Round 13 passed the `paired-interactive` version-2 native evidence checks on R2021a/R2024b/R2026a, including complete values, QC, uncertainty, and errorbar arrays. Those earlier archived reports had 3/4 native-proof coverage, with comparison still `not_verified`; that historical coverage is not a result for the failed round-17 packages. It verifies input-bound runtime declarations, not desktop interaction, full-figure appearance, or live service updates.

The comparison v3 producer in `run_matlab_gate.m` uses observation-only uncertainty and `RecordMetadata`, reading native Scatter coordinates, horizontal Line endpoints, ownership and `UserData` identities after export. The report/evaluator consumers and mutation tests check all 12 synthetic fixture records, all 11 scatter pairs, unplotted values, QC and uncertainty masks, statistics, release and input hashes. Model QC and uncertainty remain explicitly `not_provided`. Run 33996694221 failed before producing v3 evidence on all three releases; synthetic consumer tests do not establish executed 4/4 coverage. Existing packages without v3 remain compatible and unverified for comparison; malformed or mismatched declarations must fail, not bypass validation. The fixtures remain synthetic, not observed ocean evidence.

Supply external artifact checks to `build_ocean_report.py` through an explicit `--rendered-audit` file. The report validates that file's byte/hash snapshot, manifest/artifact bindings, and check/status consistency; it does not discover an audit automatically or independently rerun or authenticate the inspector. The shell workflow runs rendered-artifact inspection before report generation and passes the file explicitly. Omitting the option leaves external checks `not_verified`; an explicitly supplied missing, malformed, or inconsistent file must fail report generation. Known `pdf_font_embedding` failures on the older releases must remain visible as failures, not be hidden by unverified text or visual checks. These are external automated declarations, not a trusted visual audit. Round-14 report integration built all three release reports without modifying source CI artifacts; successful report construction does not make the reported artifact failures pass.

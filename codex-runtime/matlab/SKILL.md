...
Round-19 run 33999054663 (remote commit `85ab9d20`) completed 19/20 primary stages on each of R2021a/R2024b/R2026a, 57/60 in total. `evaluator-runtime` passed on all three; each original evaluator result has score 90 and status `runtime_pending`, not overall CI or visual approval. The corrected `SampleLabels`/`cellstr` metadata test passed. The first `test_comparison_native_evidence` adversarial run completed four positive cases and only 5/36 intended reader-negative cases per release. `Scatter.SizeData=0` then failed at the setter with `MATLAB:hg:shaped_arrays:PositiveOrNanVectorDataPredicate`, before the reader could reject it. The remaining cases, final restoration/hash assertions and suite success markers were not reached. Round 20 substitutes `NaN` for that mutation; the candidate still needs native execution and does not establish full-suite success.

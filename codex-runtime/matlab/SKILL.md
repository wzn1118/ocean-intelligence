...
Current evidence: round-23 run 34004200751 (remote commit `9593a0c`) is `completed/failure`, as confirmed by the coordinator through the GitHub API. R2021a/R2024b/R2026a each passed 20/20 primary stages, 60/60 in total. Each original evaluator result remains score 90 with `runtime_pending`; postprocessing failed and full visual approval is absent. In `family-b/astra-comparison-trial`, the `tests/model-generated-round23/` revision completed native calls, full same-figure v3 before/after export, native PNG/PDF/SVG and manifest validation on all three releases. Its independent model-artifact audits passed 2/3 on R2021a, 2/3 on R2024b and 3/3 on R2026a; both older PDFs retain `pdf_font_embedding=failed` and unembedded Courier. Faraday inspected three PNGs and three PDF renders; the three SVGs received physical/structural checks only. Older PDF renders show legend-title overflow and dashed samples crowding text; R2026a statistics crowd the top ticks and reference lines still cross points. This does not grant full visual or CJK approval; see the [native CI review](../../.codex-evals/matlab-100-20260905/astra-resume/native-ci-review-round24.md).

Historical round 22, run 34002693563 (`0f677978`), remains 58/60: R2021a/R2024b each 19/20 because the original model-source line 118 raised `astra_comparison_trial:FontUnavailable` from a listfonts assertion, not proof of a missing system font. Only the first-version R2026a trial completed native execution and its separate 3/3 artifact audit. Faraday's review of that version found complete titles/labels/legends and the PDF embeds its font, but PNG statistics touch the top ticks, vector spacing is tight with smaller text, and the reference line crosses points. This is not full visual approval or measured legend-title geometry. The revision uses `oi_font_available`; it does not rewrite the original source or clear those historical failures. Read `evals/README.md` for historical provenance.

The round-22 four-fixture `test_native_pdf_fixture_canvas` reports declare `completed_diagnostics_only` on R2021a/R2024b in primary and DISPLAY contexts, with canvas PDF calls completed. Independent offline pixel inspection nevertheless found 12 completely white restored PNGs: all four cases in R2021a primary/DISPLAY and R2024b primary. R2024b DISPLAY's four restored/reference pairs have equal decoded RGB, not PDF or visual approval. Header/hash checks missed the blank images; the candidate inspector now decodes PNGs and checks limited nonuniform foreground on white. Missing Pillow leaves pixels `not_verified`. This pixel gate does not establish restoration equivalence or authorize a production canvas strategy. Nonpublic `legend.Text.Position` remains unavailable; `captured`, matching serialized properties and completion flags do not prove restoration. R2026a remains `not_applicable`, not passed. Only strictly empty AnnotationPane objects may be excluded; nonempty ones still require rejection, which this run did not exercise. These diagnostics do not count toward the original three-candidate native PDF probe stage, score, or promoted report artifacts. Earlier JVMRequired/RootObjects failures and simple-canvas snapshots remain historical evidence in `evals/README.md`.
...
### Bounded Source Replays

The inspected Argo 4903822 snapshot has one platform, three non-continuous profiles and 1785 layers, not a complete ocean-region report. Preserve bytes/hash, profile/layer order, UTC/position, `data_keys_mode=A`, original `degree_Celsius`/`psu`/`decibar` units and all five QC flags. Pressure is not depth; do not invent uncertainty, density, named-region/nine-zone coverage, independent in-situ validation or unknown acquisition receipts. A local projection without native PNG/PDF remains `complete=false`; different inputs, releases and synthetic scores do not share approvals.

`oi_plot_ts_diagram` accepts explicit `ColorLabel="Pressure"`, `ColorUnit="decibar"`, `ColorLimits` and declared QC selection; unknown salinity/temperature types stay `unspecified` with source definitions. It has no `RawRecords`, `RecordMetadata` or comparison-v3 identity interface. The caller must retain full records, explicitly map profile-local `source_row` to concatenated call rows and bind identities in caller-managed `UserData` on each actual Scatter selected by `CompleteMask`. Read native XData/YData/CData, ownership, visibility, order and identities before/after export against source values; `SampleLabels` or copied input arrays are not native proof. Do not relabel a T-S plot as comparison v3 or invent native/visual evidence to pass report gates.

### Report Source Ownership

New report functions belong directly in `generated/<reportId>-matlab/`, derived from the server-bound report id, never a caller-selected directory. Use a legal MATLAB primary function matching its basename, such as `plot_report` in `plot_report.m`. The reserved directory accepts direct regular files with non-keyword ASCII basenames of 1-63 characters matching `[A-Za-z][A-Za-z0-9_]{0,62}` and lowercase `.m`; no subdirectories, links, data, README or export files. Legacy root `<reportId>-*.m` scripts remain compatible evidence, not a requirement to generate hyphenated function names. Manifest source references retain the full relative directory, such as `<reportId>-matlab/plot_report.m`. Source discovery does not certify syntax, execution or identity: verify `which` resolves the intended report function and do not add sibling reports to the MATLAB path. Keep input, runtime, artifact and visual gates unchanged.

### Interactive HTML Records

An offline point report has a separate wire format from a MATLAB table. In one unambiguous `application/json` point model, emit canonical `observation_id`, `temperature`, `unit`, `time`, `longitude`, `latitude` and `qc` per record. Preserve original fields, other quantities and all original QC alongside these aliases; do not change values, units or identity. PascalCase MATLAB fields alone are insufficient for the HTML inspector. Every real rendered point needs matching `data-observation-id`, zero-based `data-point-index` into the same JSON array, and aligned `data-temperature`, `data-unit`, `data-time`, `data-longitude`, `data-latitude`, `data-qc`. Keep one-based `source_row`/`source_file_row` separate and preserve a permutation for any display ordering.

Bind actual point elements to `pointerenter` and `focus`, implement visible `:hover`/`:focus-visible` states and the real detail view, then test mouse, keyboard and touch on desktop/mobile. Dense overlapping points, tooltip clipping and focus/pointer conflicts require browser checks; a static handler binding does not certify its output. Use `present`, `absent`, `unknown` or `not-evaluated` for `data-anomaly-status`, not `not_assessed`. Keep native runtime, artifact and visual acceptance pending without real evidence. HTML field compatibility and browser interaction do not certify MATLAB execution. See `INTERACTIONS.md` for the native and HTML boundaries.
...
Those historical native PDFs were content-cropped, not exact-page exports. Historical R2021a/R2024b `print` PDFs still lack embedded fonts; changing the default to WenQuanYi alone does not resolve legacy embedding or the exact-page contract. The later same-figure-canvas strategy above has separate native evidence and does not alter those originals. Validate whole-figure layout, final size, bold text, rotated Chinese labels, and PNG/SVG independently. In the two older releases, native Noto titles were `######` and native Droid Latin/numerals were boxes; neither is an equivalently verified fallback. Do not misreport backend failures as fonts being uninstalled.

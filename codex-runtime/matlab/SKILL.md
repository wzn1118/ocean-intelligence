...
Round-18 run 33997547843 completed 19/20 primary stages on each of R2021a/R2024b/R2026a, 57/60 in total. `evaluator-runtime` passed on all three; each original evaluator result has score 90 and status `runtime_pending`, not overall CI or visual approval. `family-b-runtime` failed with `test_comparison_record_metadata:MissingRejection` for `SampleLabels`: the test incorrectly expected matching `cellstr` labels to be rejected. Round 19 corrects only that test and awaits new licensed CI; the subsequent `test_comparison_native_evidence` native mutation test was not reached. Do not report that unexecuted test as passed or change the helper API to enforce the mistaken rejection.

...
drawnow;
assert_uncertainty_color_audit(result, figureHandle);
...
function assert_uncertainty_color_audit(result, figureHandle)
for handleIndex = 1:numel(result.UncertaintyGraphics)
    handle = result.UncertaintyGraphics(handleIndex);
    assert(isgraphics(handle, "line") && string(handle.Visible) == "on" ...
        && string(handle.HandleVisibility) == "off" ...
        && isappdata(handle, "OI_ColorAccessibilityRole") ...
        && isequal(string(getappdata(handle, "OI_ColorAccessibilityRole")), "uncertainty"), ...
        "test_comparison_record_metadata:UncertaintyRole", ...
        "Each identified U line must carry its explicit role separately from RecordID UserData");
end
[safe, redundant, audit] = oi_color_accessibility_audit(figureHandle);
assert(safe && redundant && audit.status == "pass" && audit.unknown_count == 0 ...
    && audit.auxiliary_count == numel(result.UncertaintyGraphics) && audit.reference_count == 1 ...
    && audit.series_count == numel(result.Scatter) && audit.data_object_count == numel(result.Scatter) ...
    && audit.object_count == numel(result.UncertaintyGraphics) + numel(result.Scatter) + 1 ...
    && ~audit.visual_inspection_verified, "test_comparison_record_metadata:ColorRoleAudit", ...
    "Only actual U segments are auxiliary; every retained scatter group must still count as data");
auxiliary = audit.objects(string({audit.objects.role}) == "auxiliary");
assert(all(string({auxiliary.type}) == "line") ...
    && all(string({auxiliary.role_basis}) == "appdata.OI_ColorAccessibilityRole"), ...
    "test_comparison_record_metadata:ColorRoleBasis", "Native U classification must use explicit role appdata");
end

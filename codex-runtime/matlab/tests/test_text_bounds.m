...
test_native_legend_title_evidence(fontName);
...
if pointsReference.succeeded
    ...
    nativePixelExtent = reshape(double(evidence.pixel_extent), 1, 4);
    pointsPixelExtent = reshape(double(pointsReference.value.pixel_extent_from_points), 1, 4);
    pointsExtentErrorPixels = abs(nativePixelExtent - pointsPixelExtent);
...
evidence.pixel_sizes = [reshape(evidence.before_export.measurement.pixel_extent(3:4), 1, 2); ...
    reshape(evidence.after_axes_export.measurement.pixel_extent(3:4), 1, 2); ...
    reshape(evidence.after_figure_export.measurement.pixel_extent(3:4), 1, 2)];
...
function test_native_legend_title_evidence(fontName)
outputRoot = string(getenv("MATLAB_FULL100_OUTPUT"));
if strlength(outputRoot) == 0
    outputRoot = string(tempname);
end
outputDirectory = fullfile(outputRoot, "text-bounds", "legend-title");
if ~isfolder(outputDirectory)
    mkdir(outputDirectory);
end
expectedFields = ["role"; "string"; "font_name"; "font_size"; "class"; "geometry_status"];
for caseName = ["visible" "empty" "hidden-title" "hidden-legend"]
    figureHandle = oi_figure(800, 500, "off");
    figureCleanup = onCleanup(@() close_if_valid(figureHandle));
    figureHandle.Units = "inches";
    figureHandle.Position(3:4) = [8 5];
    axesHandle = axes("Parent", figureHandle, "FontName", fontName);
    lineHandle = plot(axesHandle, 1:5, [1 2 1.5 3 2.5], "-o", ...
        "LineWidth", 1.5, "DisplayName", "Series");
    title(axesHandle, "Native legend title inventory", "Interpreter", "none");
    ...
    set(axesHandle, "Units", "normalized", "PositionConstraint", "outerposition", ...
        "OuterPosition", [0.08 0.10 0.84 0.80]);
    text(axesHandle, 0.3, 0.8, "Measured axes probe", "Units", "normalized", ...
        "FontName", fontName, "FontSize", 11, "Tag", "legend-measured-probe", ...
        "HorizontalAlignment", "center", "Interpreter", "none");
    legendHandle = legend(axesHandle, lineHandle, "Location", "eastoutside", ...
        "FontName", fontName, "FontSize", 10);
    nativeTitle = legendHandle.Title;
    assert(isa(nativeTitle, "matlab.graphics.illustration.legend.Text"), ...
        "test_text_bounds:LegendTitleClass", ...
        "The fixture requires the native Legend.Title, not a replacement axes Text");
    nativeTitle.String = [string(char([21335 28023])); "Legend title"];
    nativeTitle.FontName = "Courier";
    nativeTitle.FontSize = 13;
    nativeTitle.Interpreter = "none";
    nativeTitle.Visible = "on";
    if caseName == "empty"
        nativeTitle.String = "";
    elseif caseName == "hidden-title"
        nativeTitle.Visible = "off";
    elseif caseName == "hidden-legend"
        legendHandle.Visible = "off";
    end
    drawnow;
    evidence = struct("release", version('-release'), "case_name", caseName, ...
        "source", "synthetic native Legend.Title fixture", ...
        "geometry_reference", "public identity only; Legend.Title geometry remains unverified", ...
        "before_export", native_legend_title_record(legendHandle), ...
        "assertions_completed", false);
    jsonPath = fullfile(outputDirectory, caseName + ".json");
    write_evidence_json(jsonPath, evidence);
    try
        entry = oi_export_figure(figureHandle, outputDirectory, "legend-" + caseName, ...
            2400, 1500, 300, "Title", "Native legend title inventory", ...
            "Source", "synthetic native Legend.Title fixture");
    catch errorDetails
        evidence.after_export = native_legend_title_record(legendHandle);
        evidence.error_identifier = errorDetails.identifier;
        evidence.error_message = errorDetails.message;
        write_evidence_json(jsonPath, evidence);
        rethrow(errorDetails);
    end
    evidence.after_export = native_legend_title_record(legendHandle);
    evidence.entry = entry;
    write_evidence_json(jsonPath, evidence);
    assert(isequal(legendHandle.Title, nativeTitle) ...
        && evidence.after_export.string == evidence.before_export.string ...
        && evidence.after_export.visible == evidence.before_export.visible ...
        && evidence.after_export.legend_visible == evidence.before_export.legend_visible ...
        && evidence.after_export.font_size == 13, ...
        "test_text_bounds:LegendTitlePreserved", ...
        "Export must preserve the native Title, content, visibility, and font size; evidence=%s", jsonPath);

    expectedPresent = caseName == "visible";
    unmeasured = entry.unmeasured_text_objects;
    rendering = entry.rendering_evidence;
    assert(isequal(sort(string(fieldnames(unmeasured))), sort(expectedFields)) ...
        && numel(unmeasured) == double(expectedPresent) ...
        && rendering.unmeasured_count == double(expectedPresent) ...
        && rendering.bounds_audited ...
        && string(rendering.bounds_audit_scope) == "measured_objects_only" ...
        && rendering.bounds_audit_complete == ~expectedPresent ...
        && entry.publication.layout.stable == ~expectedPresent, ...
        "test_text_bounds:LegendTitleAuditScope", ...
        "Only visible nonempty native Title belongs in the unmeasured list, without bounds; evidence=%s", jsonPath);
    assert(entry.accessibility.cjk_text_present == expectedPresent ...
        && rendering.cjk_font_evidence.text_present == expectedPresent ...
        && ~rendering.cjk_font_evidence.glyph_rendering_verified ...
        && ~rendering.visual_inspection_verified ...
        && ~rendering.pdf_font_embedding_verified, ...
        "test_text_bounds:LegendTitleCJK", ...
        "Title-only CJK must enter the font inventory without implying glyph or visual verification; evidence=%s", jsonPath);
    measuredProbe = entry.text_objects(string({entry.text_objects.role}) == "legend-measured-probe");
    assert(isscalar(measuredProbe) && string(measuredProbe.string) == "Measured axes probe" ...
        && string(measuredProbe.bounds_units) == "normalized" ...
        && all(isfinite(measuredProbe.bounds)) && all(measuredProbe.bounds(3:4) > 0) ...
        && ~measuredProbe.clipped ...
        && ~any(contains(string({entry.text_objects.string}), string(char([21335 28023])))), ...
        "test_text_bounds:LegendTitleMeasuredSubset", ...
        "Ordinary axes text must retain measured bounds; native Title must not acquire fabricated bounds; evidence=%s", jsonPath);
    if expectedPresent
        assert(string(unmeasured.role) == "legend.title" ...
            && string(unmeasured.class) == "matlab.graphics.illustration.legend.Text" ...
            && string(unmeasured.string) == evidence.after_export.string ...
            && string(unmeasured.font_name) == evidence.after_export.font_name ...
            && string(unmeasured.font_name) == fontName ...
            && unmeasured.font_size == evidence.after_export.font_size ...
            && string(unmeasured.geometry_status) == "unverified" ...
            && any(string(rendering.cjk_font_evidence.selected_fonts) == string(unmeasured.font_name)), ...
            "test_text_bounds:LegendTitleIdentity", ...
            "Unmeasured evidence must match the actual native Title string, class, and final font; evidence=%s", jsonPath);
        manifestPath = fullfile(outputDirectory, "visible-manifest.json");
        manifest = oi_write_manifest(manifestPath, entry);
        assert(isequaln(manifest.figures, entry), ...
            "test_text_bounds:LegendTitleManifestIdentity", ...
            "The real writer must accept native Legend.Title evidence without refreshing any entry fields");
        serializedManifest = jsondecode(fileread(manifestPath));
        assert(isequaln(serializedManifest.figures.unmeasured_text_objects, ...
            jsondecode(jsonencode(unmeasured))) ...
            && ~serializedManifest.figures.rendering_evidence.bounds_audit_complete ...
            && serializedManifest.figures.rendering_evidence.unmeasured_count == 1, ...
            "test_text_bounds:LegendTitleManifestSerialization", ...
            "Written manifest must preserve the native Title inventory and incomplete geometry audit");
        evidence.manifest_path = manifestPath;
    end
    evidence.assertions_completed = true;
    write_evidence_json(jsonPath, evidence);
    fprintf("MATLAB_LEGEND_TITLE_TEXT_EVIDENCE=%s passed\n", caseName);
    clear figureCleanup;
    close_if_valid(figureHandle);
end
end

function record = native_legend_title_record(legendHandle)
nativeTitle = legendHandle.Title;
record = struct("legend_class", string(class(legendHandle)), ...
    "legend_visible", string(legendHandle.Visible), ...
    "class", string(class(nativeTitle)), ...
    "public_properties", string(properties(nativeTitle)), ...
    "string", strjoin(string(nativeTitle.String), " "), ...
    "font_name", string(nativeTitle.FontName), "font_size", double(nativeTitle.FontSize), ...
    "visible", string(nativeTitle.Visible));
end

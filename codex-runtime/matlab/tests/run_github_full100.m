...
[stage_records, ~] = run_stage(stage_records, "native-pdf-page-probe", ...
    @() test_native_pdf_page_probe(fullfile(output_root, "native-pdf-page-probe")), ...
    stage_status_path, expected_release);
[stage_records, ~] = run_stage(stage_records, "vector-text-alignment-probe", ...
    @() test_vector_text_alignment(fullfile(output_root, "vector-text-alignment-probe")), ...
    stage_status_path, expected_release);
...
[stage_records, ~] = run_stage(stage_records, "comparison-statistics-layout", ...
    @() test_comparison_statistics_layout(output_root), stage_status_path, expected_release);
[stage_records, ~] = run_stage(stage_records, "hovmoller-time-axis", ...
    @() test_hovmoller_time_axis(fullfile(output_root, "hovmoller-time-axis")), ...
    stage_status_path, expected_release);
...
[stage_records, ~] = run_stage(stage_records, "astra-argo-native", ...
    @() test_astra_generated_argo(output_root), stage_status_path, expected_release);
...
diagnose_svg_print_sizes(fullfile(export_directory, "svg-print-sizes-probe"));

...
report = struct("schema_version", 1, "status", "running", ...
    ...
    "fixture_canvas_report", "native-fixture-canvas/native-fixture-canvas.json", ...
    "fixture_canvas_status_scope", "independent old-release fixture diagnostic; not counted in this stage", ...
    "fixture_canvas_restore_report", "fixture-canvas-restore/fixture-canvas-restore-probe.json", ...
    "fixture_canvas_restore_scope", "read separate JSON and stderr; same-process diagnostic, not main three candidates, stages or score", ...
    "comparison_legend_tile_report", "comparison-legend-tile/comparison-legend-tile-probe.json", ...
    "comparison_legend_tile_scope", "independent single-variable layout diagnostic; not counted in this stage", ...
...
try
    report.fixture_canvas_experiment = test_native_pdf_fixture_canvas( ...
        fullfile(outputDirectory, "native-fixture-canvas"));
catch errorRecord
    fprintf(2, "MATLAB_NATIVE_FIXTURE_CANVAS_PROBE_FAILED=%s: %s\n", ...
        errorRecord.identifier, errorRecord.message);
end
try
    test_comparison_legend_tile_probe(fullfile(outputDirectory, "comparison-legend-tile"));
catch errorRecord
    fprintf(2, "MATLAB_COMPARISON_LEGEND_TILE_PROBE_FAILED=%s: %s\n", ...
        errorRecord.identifier, errorRecord.message);
end
try
    test_fixture_canvas_restore_probe(fullfile(outputDirectory, "fixture-canvas-restore"));
catch errorRecord
    fprintf(2, "MATLAB_FIXTURE_CANVAS_RESTORE_PROBE_FAILED=%s: %s\n", ...
        errorRecord.identifier, errorRecord.message);
end

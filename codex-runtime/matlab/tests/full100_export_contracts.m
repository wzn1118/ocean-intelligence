...
beforePath = fullfile(outputDirectory, "publication-before-export.png");
print(figureHandle, char(beforePath), "-dpng", "-r300");
...
assert(isequal(figureHandle.Children, originalChildren) ...
    && isequal(figureHandle.CurrentAxes, originalCurrentAxes), ...
    "full100_export_contracts:GraphicsOwnership", "Export must preserve original graphics and current axes");
assert(isempty(findall(figureHandle, "Tag", "OI_PdfCanvasBackground")), ...
    "full100_export_contracts:CanvasCleanup", "Temporary PDF canvas must be removed");
afterPath = fullfile(outputDirectory, "publication-after-export.png");
print(figureHandle, char(afterPath), "-dpng", "-r300");
assert(isequal(imread(beforePath), imread(afterPath)), ...
    "full100_export_contracts:RestoredPixels", "Export must preserve the original rendered pixels");
fprintf("MATLAB_EXPORT_ORIGINAL_PIXELS_PRESERVED=true\n");
delete(beforePath);
delete(afterPath);
...
if ~verLessThan('matlab', '25.1')
    ...
    assert(~entry.runtime.exact_exportgraphics_available ...
        ...
        "Legacy releases must retain exact PNG print sizing and native font-embedding PDF export");
end
...
test_native_raster_aspect(outputDirectory, theme);
test_raster_dimensions(outputDirectory, theme);
test_direction_rose_export(fullfile(outputDirectory, "direction-rose-export"));
fprintf("MATLAB_FULL100_EXPORT_CONTRACTS=passed\n");
end

function test_native_raster_aspect(outputDirectory, theme)
sizes = [400 300 150; 1200 675 180; 997 613 300];
angles = linspace(0, 2 * pi, 257);
for caseIndex = 1:size(sizes, 1)
    ...
    lineHandle = plot(axesHandle, cos(angles), sin(angles), "-", ...
        "Color", [0 0.447 0.698], "LineWidth", 1.5);
    axis(axesHandle, "equal");
    xlim(axesHandle, [-1.25 1.25]);
    ylim(axesHandle, [-1.25 1.25]);
    xlabel(axesHandle, "X (1)", "Interpreter", "none");
    ylabel(axesHandle, "Y (1)", "Interpreter", "none");
    title(axesHandle, "Equal data scale", "Interpreter", "none");
    oi_apply_axes(axesHandle, theme);
    drawnow;
    nativeX = lineHandle.XData;
    nativeY = lineHandle.YData;
    dataAspectRatio = axesHandle.DataAspectRatio;
    fontSizes = [axesHandle.FontSize axesHandle.Title.FontSize ...
        axesHandle.XLabel.FontSize axesHandle.YLabel.FontSize];
    caseDirectory = fullfile(outputDirectory, "raster-aspect-" + string(width) + "-" + string(height));
    entry = oi_export_figure(figureHandle, caseDirectory, "equal-data-scale", ...
        width, height, dpi, "Title", "Equal data scale", ...
        "Source", "Synthetic native raster aspect contract", "Theme", theme.Name);
    assert(isequaln(lineHandle.XData, nativeX) && isequaln(lineHandle.YData, nativeY) ...
        && isequal(axesHandle.DataAspectRatio, dataAspectRatio) ...
        && isequal([axesHandle.FontSize axesHandle.Title.FontSize ...
            axesHandle.XLabel.FontSize axesHandle.YLabel.FontSize], fontSizes), ...
        "full100_export_contracts:RasterSourceChanged", ...
        "Native raster export must preserve source arrays, equal data scale and point sizes");
    raster = double(imread(fullfile(caseDirectory, entry.exports.png.file))) / 255;
    assert(size(raster, 3) == 3, "full100_export_contracts:RasterRgb", ...
        "Aspect diagnostic requires a truecolor native PNG");
    curveMask = raster(:, :, 3) > raster(:, :, 2) + 0.1 ...
        & raster(:, :, 2) > raster(:, :, 1) + 0.2;
    [curveRows, curveColumns] = find(curveMask);
    assert(numel(curveRows) >= 100, "full100_export_contracts:RasterCurveMissing", ...
        "The actual native PNG must contain the blue unit circle");
    circlePixels = [max(curveColumns) - min(curveColumns) + 1 ...
        max(curveRows) - min(curveRows) + 1];
    assert(abs(diff(circlePixels)) <= 2, "full100_export_contracts:RasterAspectDistortion", ...
        "Equal data scale rendered as %dx%d pixels; width/height may differ by at most two edge pixels", ...
        circlePixels(1), circlePixels(2));
    oi_write_manifest(fullfile(caseDirectory, "figures.json"), entry);
    fprintf("MATLAB_RASTER_ASPECT=%s\n", jsonencode(struct("figure_id", entry.id, ...
        "target_pixels", [width height], "dpi", dpi, "circle_pixels", circlePixels, ...
        "source_unchanged", true, "whole_figure_visual_verified", false)));
    clear figureCleanup;
end
end
...
for caseIndex = 1:size(sizes, 1)
    ...
    assert(abs(entry.exports.pdf.physical_width_in - entry.exports.pdf.width / 72) <= 1e-9 ...
        && abs(entry.exports.pdf.physical_height_in - entry.exports.pdf.height / 72) <= 1e-9, ...
        "full100_export_contracts:ActualPdfDimensions", ...
        "PDF physical-size evidence must use the actual native MediaBox, not the requested size");
    assert(entry.runtime.export_size_units.png == "inches" ...
    ...
    invalidPhysicalEntry = entry;
    invalidPhysicalEntry.exports.pdf.physical_width_in = entry.exports.pdf.physical_width_in + 1 / 720;
    invalidPhysicalPath = fullfile(caseDirectory, "invalid-physical-size.json");
    assert_throws(@() oi_write_manifest(invalidPhysicalPath, invalidPhysicalEntry), "InvalidPdf");
    assert(~isfile(invalidPhysicalPath), ...
        "full100_export_contracts:TamperedManifest", ...
        "Even a sub-point mismatch between PDF metadata and actual geometry must not produce a manifest");
    oi_write_manifest(fullfile(caseDirectory, "figures.json"), entry);
